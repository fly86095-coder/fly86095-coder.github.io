...
function footer(){return `<footer class="footer"><span>本網站由 <b>Han</b> 製作。內容可自由用於個人學習與非商業教育用途；未經授權，不得作商業使用。</span></footer>`}
...
function copyArticle(slug,level,button){
 ...
 const mode=state.display;const lines=[v.title];
 if(mode!=='en'&&v.titleZh)lines.push(v.titleZh);
 if(mode==='all'&&v.titlePinyin)lines.push(v.titlePinyin);
 lines.push('',`Level: ${level}${v.degree?` · ${v.degree}`:''}`,'');
 (v.sections||[]).forEach(s=>{
   lines.push(s.heading,s.en);
   if(mode!=='en')lines.push(s.zh);
   if(mode==='all')lines.push(s.pinyin||pinyinText(s.zh));
   lines.push('');
 });
