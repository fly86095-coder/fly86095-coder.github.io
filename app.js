...
function home(){const latest=[...COUNTRIES].sort((a,b)=>b.readOrder-a.readOrder).slice(0,4);const content=`<section class="hero"><div><div class="eyebrow">Bilingual English Reading Practice Atlas</div><h1>Read the World<br>One Country at a Time</h1><p>Build your English through country-based reading practice. Explore geography, culture, history, food, and everyday life with graded A1, A2, and B1 readings. Switch between English only, English + Simplified Chinese, or English + Simplified Chinese + word-grouped pinyin to match the way you want to learn.</p><div class="stats">${stat(COUNTRIES.length,'Country entries')}${stat(articleCount(),'Reading versions')}${stat(SITE.regions.length,'Practice regions')}</div></div><div class="map-card"><svg id="mapSvg"></svg><div class="legend"><div><span class="swatch read"></span>Read</div><div><span class="swatch"></span>Not yet read</div><small>Zoom in to reveal country names</small></div><div class="map-tools"><button onclick="mapZoom(1.45)">+</button><button onclick="mapZoom(.69)">−</button><button onclick="mapReset()">↺</button></div></div></section><section class="section"><div class="section-head"><div><h2>Latest Countries</h2><p>Choose any available reading level before you enter.</p></div><button class="outline" onclick="go('#library')">Open full library →</button></div><div class="country-grid">${latest.map(card).join('')}</div></section><section class="section"><div class="section-head"><div><h2>Browse by Region</h2><p>The archive only includes countries you have already read.</p></div></div><div class="region-tabs">${['All',...SITE.regions].map(r=>`<button class="tab" onclick="state.region='${esc(r)}';state.page=1;go('#library');render()">${esc(r)}</button>`).join('')}</div></section>`;shell(content,'Home');hydrate();renderMap()}
