...
function footer(){return `<footer class="footer"><span class="footer-en">This website was created by <b>Han</b>. Content may be freely used for learning and non-commercial educational purposes; commercial use requires prior authorization.</span><span class="footer-zh">本網站由 <b>Han</b> 製作。內容可自由用於學習與非商業教育用途；未經授權，不得作商業使用。 · ${SITE.version}</span></footer>`}
...
function learningWords(v,level){
 if(Array.isArray(v.learningWords)&&v.learningWords.length)return v.learningWords;
...
function vocabCard(words){
 ...
 return `<div class="vocab-list">${words.map((w,i)=>{const py=w.pinyin||pinyinText(w.zh);return `<button class="vocab-row" type="button" onclick="showWordPopup(event,${i},'en')"><span class="vocab-en"><b>${esc(w.word)}</b><small>${esc(w.kk)}</small></span><span class="vocab-zh">${esc(w.zh)} <small>${esc(py)}</small></span></button>`}).join('')}</div>`
}
...
function showWordPopup(event,index,lang='en'){
 ...
 const py=w.pinyin||pinyinText(w.zh);
