...
function home(){
 ...
 <section class="practice-home-hero">
  <div class="practice-home-copy">
   ...
   <p>Choose the kind of English you want to train. Country & Culture Reading remains your graded reading path, while Daily Conversation Practice adds reusable speaking patterns for real life. More practice types can be added without changing the material you already have.</p>
   <div class="practice-home-actions"><button class="primary" onclick="go('#practice')">Explore practice paths</button><button class="outline" onclick="go('#dialogues')">Try daily conversation</button></div>
   <div class="stats">${stat(2,'Active practice paths')}${stat(COUNTRIES.length,'Country entries')}${stat(dialogueCount,'Conversation sets')}</div>
  </div>
  <div class="practice-home-guide">
   <div class="eyebrow">How to use the site</div>
   <ol><li><b>Choose a practice path.</b><span>Reading, conversation, and future training modes live under one framework.</span></li><li><b>Choose your level or topic.</b><span>Start where the material feels useful, not where it feels impressive.</span></li><li><b>Practice actively.</b><span>Read, speak, replace blanks, use hints only when needed, and make the English personal.</span></li></ol>
  </div>
 </section>
 <section class="section"><div class="section-head"><div><h2>Practice Paths</h2><p>Country culture is now one branch of a larger English practice system.</p></div><button class="outline" onclick="go('#practice')">Open practice hub →</button></div>${practicePathCards(true)}</section>
 <section class="section"><div class="section-head"><div><div class="eyebrow">Country & Culture Reading</div><h2>Keep Reading the World</h2><p>Your existing country articles, levels, map, word help, and reading journey stay intact.</p></div><button class="outline" onclick="go('#library')">Browse countries →</button></div><div class="country-home-grid"><div class="country-home-copy"><h3>${COUNTRIES.length} country entries</h3><p>Choose A1, A2, or B1 where available, then switch between English, Simplified Chinese, and word-grouped pinyin while you read.</p><div class="mini-metrics"><span>${articleCount()} reading versions</span><span>${SITE.regions.length} regions</span></div></div><div class="map-card"><svg id="mapSvg"></svg><div class="legend"><div><span class="swatch read"></span>Read</div><div><span class="swatch"></span>Not yet read</div><small>Zoom in to reveal country names</small></div><div class="map-tools"><button onclick="mapZoom(1.45)">+</button><button onclick="mapZoom(.69)">−</button><button onclick="mapReset()">↺</button></div></div></div></section>
 ...
 <section class="section"><div class="section-head"><div><h2>Browse by Region</h2><p>The archive only includes countries you have already read.</p></div></div><div class="region-tabs">${['All',...SITE.regions].map(r=>`<button class="tab" onclick="state.region='${esc(r)}';state.page=1;go('#library');render()">${esc(r)}</button>`).join('')}</div></section>`;
...
function practicePathCards(compact=false){
 ...
 return `<div class="practice-path-grid ${compact?'compact':''}">${paths.map(p=>{
  ...
  return `<article class="practice-path-card ${active?'active':'soon'}" ${active?`onclick="go('${p.route}')" role="button" tabindex="0"`:''}><div class="path-top"><span class="path-kicker">${esc(p.kicker)}</span><span class="path-status ${active?'live':''}">${active?'Available':'Coming soon'}</span></div><h3>${esc(p.title)}</h3><div class="path-zh">${esc(p.titleZh||'')}</div><p>${esc(p.desc)}</p><small>${esc(p.meta||'')}</small>${active?'<div class="path-arrow">Open →</div>':''}</article>`
 }).join('')}</div>`
}
function practiceHub(){
 const content=`<section class="page-pad practice-page-head"><div class="eyebrow">Practice hub</div><h1 class="page-title">Choose What You Want to Practice</h1><p>Country articles are now one practice path, not the whole site. Start with the skill you want today; future practice modes can plug into the same structure without changing your existing materials.</p></section>
 <section class="practice-how"><div class="practice-how-step"><span>1</span><div><b>Choose a path</b><p>Reading or daily conversation for now.</p></div></div><div class="practice-how-step"><span>2</span><div><b>Choose a level or topic</b><p>A1, A2, B1, country, or real-life situation.</p></div></div><div class="practice-how-step"><span>3</span><div><b>Do something with the English</b><p>Read it, say it, replace it, then make it your own.</p></div></div></section>
 <section class="section practice-section"><div class="section-head"><div><h2>Practice Paths</h2><p>Two paths are active. The other cards reserve clean space for later expansion.</p></div></div>${practicePathCards(false)}</section>
 <section class="section practice-principle"><div class="section-head"><div><h2>One Site, Different Training Modes</h2><p>The structure separates <b>what you practice</b> from <b>how you practice</b>. Country reading can keep growing independently while conversation, translation, listening, and recall tools are added later.</p></div></div></section>`;
 shell(content,'Practice')
}
function blankTypeLabel(type){return type==='information'?'Personal / Information Blank':type==='pattern'?'Sentence Pattern Blank':'Vocabulary Blank'}
function dialogueBlankCount(d){return (d.turns||[]).filter(t=>t.blankType).length}
function dialogueCard(d){return `<article class="dialogue-card" onclick="go('#dialogue=${d.id}')" role="button" tabindex="0"><div class="dialogue-card-top"><span class="level-pill ${d.level.toLowerCase()}">${d.level}</span><span>${esc(d.scene)}</span></div><h3>${esc(d.title)}</h3><div class="dialogue-title-zh">${esc(d.titleZh)}</div><p>${esc(d.goal)}</p><div class="dialogue-card-meta"><span>${dialogueBlankCount(d)} blanks</span><span>${(d.turns||[]).length} turns</span></div><div class="dialogue-open">Start practice →</div></article>`}
function dialogueLibrary(level=dialogueLevelState){
 ...
 const content=`<section class="page-pad practice-page-head"><div class="breadcrumbs"><a href="#home">Home</a> / <a href="#practice">Practice</a> / Daily Conversation</div><div class="eyebrow">Daily Conversation Practice</div><h1 class="page-title">Build a Reusable Speaking Skeleton</h1><p>Each set is semi-open: keep the core sentence pattern, replace only the useful parts, and speak before you reveal the answer. The goal is not to memorize one dialogue—it is to reuse the same structure in real life.</p></section>
 <section class="conversation-method"><div><span class="method-n">1</span><b>Read the blank</b><p>Try to complete it from memory first.</p></div><div><span class="method-n">2</span><b>Open the hint only if needed</b><p>Choose a word, real detail, or sentence pattern.</p></div><div><span class="method-n">3</span><b>Say the line out loud</b><p>Use the whole sentence, not only the missing words.</p></div><div><span class="method-n">4</span><b>Make your own version</b><p>Replace the example with something true about you.</p></div></section>
 <section class="section dialogue-library"><div class="section-head"><div><h2>Choose a Level</h2><p>Five clean practice sets at each level.</p></div><span class="dialogue-count">${list.length} sets</span></div><div class="dialogue-level-tabs">${['All','A1','A2','B1'].map(l=>`<button class="tab ${level===l?'active':''}" onclick="dialogueLibrary('${l}')">${l}</button>`).join('')}</div><div class="dialogue-grid">${list.map(dialogueCard).join('')}</div></section>`;
 shell(content,'Practice')
}
function hintDetails(t){
 if(!t.blankType||!t.hints?.length)return '';
 return `<div class="blank-tools"><span class="blank-type">${blankTypeLabel(t.blankType)}</span><details class="hint-details"><summary>Show hints / 提示</summary><div class="hint-list">${t.hints.map(h=>`<span><b>${esc(h.en)}</b><small>${esc(h.zh)}</small></span>`).join('')}</div></details></div>`
}
function dialoguePage(id){
 ...
 const turns=(d.turns||[]).map((t,idx)=>`<div class="dialogue-turn"><div class="speaker speaker-${t.speaker.toLowerCase()}">${esc(t.speaker)}</div><div class="turn-body"><div class="turn-en">${esc(t.en)}</div><div class="turn-zh">${esc(t.zh)}</div>${hintDetails(t)}</div></div>`).join('');
 const example=(d.example||[]).map(t=>`<div class="example-line"><b>${esc(t.speaker)}:</b><div><span>${esc(t.en)}</span><small>${esc(t.zh)}</small></div></div>`).join('');
 const your=(d.yourVersion||[]).map(x=>`<li>${esc(x)}</li>`).join('');
 const content=`<section class="dialogue-detail"><div class="breadcrumbs"><a href="#home">Home</a> / <a href="#practice">Practice</a> / <a href="#dialogues">Daily Conversation</a> / ${esc(d.title)}</div><div class="dialogue-detail-head"><div><div class="eyebrow">${esc(d.scene)}</div><div class="detail-title-line"><span class="level-pill ${d.level.toLowerCase()}">${d.level}</span><h1>${esc(d.title)}</h1></div><div class="detail-title-zh">${esc(d.titleZh)}</div><p>${esc(d.goal)}</p></div><aside class="practice-rule"><b>Best way to use this</b><ol><li>Answer before opening hints.</li><li>Say the full line aloud.</li><li>Open the example only after one attempt.</li><li>Finish with your own real version.</li></ol></aside></div>
 <div class="dialogue-practice-card"><div class="dialogue-card-heading"><div><div class="eyebrow">Conversation</div><h2>Blank → Hint → Example</h2></div><span>${dialogueBlankCount(d)} replaceable blanks</span></div>${turns}</div>
 <details class="complete-example"><summary>Show complete example / 完整范例</summary><div class="complete-example-body">${example}</div></details>
 <section class="your-version-box"><div><div class="eyebrow">Your version</div><h2>Make the dialogue about your real life</h2><ul>${your}</ul></div><textarea class="your-version-input" rows="6" placeholder="Write only the lines you want to change. Then say them out loud…"></textarea></section>
